...
class RedirectIfOwnerNotExist
{
    ...
    public function handle(Request $request, Closure $next)
    {
        ...
        if (empty($ownerIdentifier) || empty($owner = Owner::where(['identifier' => $request->header('X-OwnerIdentifier')])->with(['realEstates'=>function($query){$query->where('deleted_at',null)->orderBy('created_at','DESC');}])->first())) {
