...
class RedirectIfOwnerNotExist
{
    ...
    public function handle(Request $request, Closure $next)
    {
        ...
        if (empty($ownerIdentifier) || empty($owner = Owner::where(['identifier' => $request->header('X-OwnerIdentifier')])->with(['realEstates'=>function($query){$query->orderBy('created_at','DESC');}])->first())) {
