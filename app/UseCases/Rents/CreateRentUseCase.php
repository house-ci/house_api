...
//                if ($nbrMonth > 0 && !empty($rentForNext)) {
                $month = $rents->month;
                $year = $rents->year;
                $day = 1;
...
//
//                $lastLeasingEndDate = (new DateTime($lastLeasingDate))->add(new   DateInterval('P'.$asset->frequency_in_month.'M'))->format('Y-m-t');
//                // add 7 days to the date
//                $rentForPay = Helpers::getDayOfMonth($paymentDeadlineDay, $lastLeasingDate, $lastLeasingEndDate,$asset->frequency_in_month);
////            dd($rents,$asset,$rentForPay,$leasing->next_leasing_period);
//            }
            ...
            foreach ($rentForPay as $rentDeadLine) {
                ...
                $label = '#.'.$leasing->type.'.' . $leasing->id . "-$rentMonth-$rentYear";
